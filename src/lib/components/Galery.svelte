	...
	<div class="relative z-10" in:fade>
		...
		>
			<div class="absolute inset-0 flex items-center justify-center">
				<img
					loading="lazy"
					src={photos[activeIndex]?.url + '?auto=compress&q=60'}
					alt=""
				...
				>
				<button
					class="hidden sm:block absolute md:bottom-1/2 md:left-0 sm:bottom-12 bottom-24 left-4 m-4 text-white hover:text-gray-400 text-3xl"
					on:click={prev}>&#x2329;</button
				>
				<button
					class="hidden sm:block absolute md:bottom-1/2 md:right-0 sm:bottom-12 bottom-24 right-4 m-4 text-white hover:text-gray-400 text-3xl"
					on:click={next}
				>
					&#x232a;
				</button>
